...
class AppConfig:
    ...
    def load_sys_config(cls, file_path=None):
        ...
        with open(file_path, 'r') as sys_config_file:
            ...
            cls.fallback_csv_path = str(Path(os.path.expanduser(cls.system_config['output']['fallback_csv'])))
    ...
    def validate_configurations(cls, config):
        ...
        for key in necessary_keys:
            if key not in config:
                logger.error(f"{key.capitalize()} configuration is missing in the configuration data.")
                raise ValueError(f"{key.capitalize()} configuration is missing in the configuration data.")
            else: 
    ...
    def load_keywords(cls, file_path=None):
        ...
        with open(file_path, 'r') as keywords_file:

...
class ProgressWindow:
    def __init__(
        ...
        label_format="{}/{}",
        log_message="Total items identified: {}",
    ):
        ...
        self.label = tk.Label(
            self.root, text=self.label_format.format(0, self.total_items)
        )
        self.label.pack()

        # Create the progress bar
        self.progressbar = ttk.Progressbar(
            self.root, length=400, mode="determinate", maximum=self.total_items
        )
    ...
    def update_progress(self, increment=1):
        ...
        self.label.config(
            text=self.label_format.format(self.progress, self.total_items)
        )
        self.root.update()

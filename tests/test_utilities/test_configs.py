...
def mock_open_keywords():
    with mock.patch(
        "builtins.open", mock.mock_open(read_data='["keyword1", "keyword2"]')
    ):
        yield
...
def test_load_keywords(mock_open_keywords, caplog):
    ...
    # Check log messages
    assert any(
        "Loading keywords from keywords.json" in message for message in caplog.messages
    )


def test_load_user_config(mock_open_user_config, caplog):
    with mock.patch(
        "utilities.configs.AppConfig.validate_configurations"
    ) as mock_validate:
...
def test_validate_configurations_missing_key(caplog):
    config = {"postgres": {}, "elasticsearch": {}, "sqlite": {}, "keywords": {}}

    with caplog.at_level(logging.ERROR):
        with pytest.raises(
            ValueError,
            match="Logging configuration is missing in the configuration data.",
        ):
            AppConfig.validate_configurations(config)

    # Check log messages
    assert any(
        "Logging configuration is missing in the configuration data." in message

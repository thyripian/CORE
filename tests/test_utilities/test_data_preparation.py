...
info = {
    ...
    "subjects": [
        {"text": "Subject1", "label": "Label1"},
        {"text": "Subject2", "label": "Label2"},
    ],
...
def test_prepare_data(caplog):
    with caplog.at_level(logging.INFO):
        with mock.patch(
            "data_processing.text_processing.clean_timeframes",
            return_value="2024-06-04,2024-06-05",
        ), mock.patch(
            "data_processing.text_processing.remove_duplicate_locations",
            return_value="Location1,Location2",
        ), mock.patch(
            "data_processing.text_processing.filter_text",
            side_effect=lambda x: f"Filtered_{x}",
        ), mock.patch(
            "utilities.data_preparation.filter_individual_topics"
            ...
            # Verify log messages
            assert any(
                "Preparing extracted data for storage." in message
                for message in caplog.messages
            )
            assert any(
                "Precleaned timeframes: ['2024-06-04', '2024-06-05']" in message
                for message in caplog.messages
            )
            assert any(
                "Cleaned timeframes: 2024-06-04,2024-06-05" in message
                for message in caplog.messages
            )
            assert any(
                "SUBJECTS STR TYPE: <class 'str'>" in message
                for message in caplog.messages
            )
            assert any(
                "SUBJECTS STR CONTENT: Subject1:Label1|Subject2:Label2" in message
                for message in caplog.messages
            )

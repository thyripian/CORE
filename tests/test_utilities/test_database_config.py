...
def mock_config_manager():
    with mock.patch("initialization.config_manager.ConfigManager.get_user_config") as mock_get_config:
...
def test_set_elastic_conn_data(mock_config_manager, caplog):
    # Test with a mock configuration
    mock_config_manager.return_value = {'host': 'localhost', 'port': 9200, 'index': 'test_index'}
    DatabaseConfig.set_elastic_conn_data()
    assert DatabaseConfig.elastic_conn_data == {'host': 'localhost', 'port': 9200, 'index': 'test_index'}

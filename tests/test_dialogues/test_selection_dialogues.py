...
def test_show_info_dialog(mock_tk):
    with mock.patch(
        "utilities.dialogues.selection_dialogues.messagebox.showinfo"
    ) as mock_showinfo:
...
def test_select_folder(mock_tk):
    with mock.patch(
        "utilities.dialogues.selection_dialogues.filedialog.askdirectory"
    ) as mock_askdirectory:
        with mock.patch(
            "utilities.dialogues.selection_dialogues.show_info_dialog"
        ) as mock_show_info_dialog:
            mock_askdirectory.return_value = "/path/to/folder"
            folder = select_folder()
            mock_show_info_dialog.assert_called_with(
                "Folder Selection",
...
def test_select_file(mock_tk):
    with mock.patch(
        "utilities.dialogues.selection_dialogues.filedialog.askopenfilename"
    ) as mock_askopenfilename:
        with mock.patch(
            "utilities.dialogues.selection_dialogues.show_info_dialog"
        ) as mock_show_info_dialog:
...
def test_confirm_selection(mock_tk):
    with mock.patch(
        "utilities.dialogues.selection_dialogues.messagebox.askyesno"
    ) as mock_askyesno:

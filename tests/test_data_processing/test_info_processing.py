...
from data_processing.text_processing import (
    extract_classification_and_caveats, extract_keywords, extract_topics,
    filter_text, normalize_mgrs, unique_subjects)
...
def test_process_data(
    mock_app_init, mock_extract_info, mock_hash_checker, mock_db_manager, setup_files
):
...
@patch("data_processing.data_processing_manager.HashChecker")
@patch(
    "data_processing.data_processing_manager.progress_window.ProgressWindow",
    new=MagicMock,
)
def test_process_data_with_empty_files(mock_hash_checker, mock_app_init, setup_files):
    ...
    with patch(
        "data_extraction.text_extractors.extract_text_from_word",
        side_effect=PackageNotFoundError,
    ):
        with patch(
            "data_extraction.text_extractors.extract_text_from_excel",
            side_effect=BadZipFile,
        ):
            with patch(
                "data_extraction.text_extractors.extract_text_from_pptx",
                side_effect=PptxPackageNotFoundError,
            ):
                with patch(
                    "data_extraction.text_extractors.extract_text_from_pdf",
                    side_effect=pdfminer.pdfparser.PDFSyntaxError,
                ):
...
def test_process_data_with_corrupted_files(
    mock_hash_checker, mock_app_init, setup_files
):
    mock_hash_checker.check_hash_exists.return_value = False
    mock_app_init.return_value = MagicMock()
...
def test_extract_info_empty_text(mock_extract_topics, mock_app_init):
    ...
    assert (
        info["highest_classification"] == "none_found"
    ), f"Expected 'none_found' for classifications, got {info['classifications']}"
    assert (
        info["caveats"] == "none_found"
    ), f"Expected 'none_found' for caveats, got {info['caveats']}"

...
def test_initialize_application():
    with mock.patch.object(InitElastic, "elastic_init_interaction"), mock.patch.object(
...
def test_load_configurations():
    with mock.patch.object(
        DatabaseConfig, "__init__", return_value=None
    ), mock.patch.object(
        DatabaseConfig,
        "availability",
        {"postgresql": True, "elasticsearch": True, "sqlite": True, "fallback": True},
    ), mock.patch.object(
        AppConfig, "load_sys_config"
    ), mock.patch.object(
        AppInitialization, "setup_logging"
    ), mock.patch(
        "initialization.init_app.select_file", return_value="mock_config_file"
    ), mock.patch(
        "initialization.init_app.confirm_selection", return_value=True
    ), mock.patch(
        "builtins.open", mock.mock_open(read_data='{"some_key": "some_value"}')
    ), mock.patch.object(
        AppConfig, "load_user_config"
    ), mock.patch.object(
        AppConfig, "nlp", "mock_nlp_model"
    ):
        AppInitialization.load_configurations()

        assert AppInitialization.availability == {
            "postgresql": True,
            "elasticsearch": True,
            "sqlite": True,
            "fallback": True,
...
def test_setup_logging():
    ...
    ), mock.patch("utilities.setup_logging"), mock.patch("utilities.init_logging"):
        AppInitialization.setup_logging()
...
def test_run_connection_tests():
    with mock.patch.object(
        DatabaseManager, "__init__", return_value=None
    ), mock.patch.object(
        DatabaseManager, "test_initial_connections"
    ), mock.patch.object(
        DatabaseManager, "connection_status", {"status": "ok"}
    ):
        AppInitialization.run_connection_tests()
...
def test_set_database_connections():
    with mock.patch.object(
        AppInitialization, "setup_postgres_conn", return_value=True
    ), mock.patch.object(
        AppInitialization, "setup_elastic_conn", return_value=True
    ), mock.patch.object(
        AppInitialization, "setup_sqlite_conn", return_value=True
    ):
        AppInitialization.set_database_connections()


def test_setup_postgres_conn():
    with mock.patch.object(DatabaseConfig, "set_postgres_conn_data"), mock.patch.object(
        DatabaseManager, "try_postgresql_connection", return_value=True
    ):
        assert AppInitialization.setup_postgres_conn()


def test_setup_elastic_conn():
    with mock.patch.object(DatabaseConfig, "set_elastic_conn_data"), mock.patch.object(
        DatabaseManager,
        "get_elasticsearch_client",
        return_value=mock.Mock(ping=lambda: True),
...
def test_select_processing_directory():
    with mock.patch(
        "initialization.init_app.select_folder", return_value="mock_dir"

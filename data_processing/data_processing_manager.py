...
from data_extraction.image_extractors import extract_images_from_excel, extract_images_from_pdf, extract_images_from_pptx, extract_images_from_word
from data_extraction.text_extractors import extract_text_from_excel, extract_text_from_pdf, extract_text_from_pptx, extract_text_from_word
...
class DataProcessingManager:
    ...
    file_type_mapping = {
        'docx': {
            ...
            'image': extract_images_from_word
        },
        'xlsx': {
            'text': extract_text_from_excel,
            'image': extract_images_from_excel
        },
        'pptx': {
            'text': extract_text_from_pptx,
            'image': extract_images_from_pptx
        },
        'pdf': {
            'text': extract_text_from_pdf,
            'image': extract_images_from_pdf
        }
    }
    ...
    def __init__(self):
        ...
        logger.info(f'The DataProcessingManager has identified the following library availabilities: \n\t{available}')
        logger.info(f'The DataProcessingManager has identified the following available database connections: \n\t{connects}')
            ...
            for ext, files in file_path_dict.items():
                for file_path in files:
                    ...
                    if HashChecker.check_hash_exists(file_hash):
                        logger.info(f"Skipping {filename}, already processed / exists in database.")
                        continue

                    file_type = file_path.split('.')[-1]  # Assume the file type is the extension
                    extractor = cls.file_type_mapping.get(file_type)
                    
                    if extractor:
                        text_data = extractor['text'](file_path) if 'text' in extractor else None
                        image_data = extractor['image'](file_path) if 'image' in extractor else None
                        ...
                        if image_data:
                            try:
                                image_data = pickle.dumps(image_data) if image_data else None
                            except Exception as e:
                                logger.info(f"Error attempting image compression: {e}")

                    processed_data = cls.preprocess_data(file_path,extracted_data,image_data,file_hash)
    ...
    def preprocess_data(cls, file_path, data, image_data, file_hash):
        processed_data = data_preparation.prepare_data(file_path, data, image_data, file_hash)

...
# # Directory traversal function to identify and categorize files by their extensions.
...
#         for file in files:
#             full_path = os.path.join(root, file)  # Combine root path with file name
#             ext = file.split('.')[-1]  # Extract the file extension
#             if ext in file_dict.keys():  # Check if the file extension is one we're looking for
#                 file_dict[ext].append(full_path)  # Add the file to the corresponding list

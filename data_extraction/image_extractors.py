...
def extract_images_from_word(file_path):
    ...
    for rel_id, rel in doc.part.rels.items():
        ...
        if "image" in rel.reltype:
            ...
            image_bytes_io = BytesIO(image_bytes)  # Create a BytesIO object from the image bytes
...
def extract_images_from_excel(file_path):
    ...
    with ZipFile(file_path, 'r') as zip_ref:
        zip_ref.extractall(temp_dir)  # Extract all contents into the temporary directory
...
def extract_images_from_pptx(file_path):
    ...
    for slide in prs.slides:
        ...
        for shape in slide.shapes:
            if shape.shape_type == 13:  # Check if the shape is an image (13 is the code for image shape)
                image_bytes = shape.image.blob  # Get the image blob from the shape
                image_bytes_io = BytesIO(image_bytes)  # Create a BytesIO object from the blob
                compressed_image = compress_image(image_bytes_io)  # Compress the image

...
def update_progress(current, total, message):
    ...
    logger.info(f"Progress Updated: {progress_state}")  # Add print to confirm it's being called
...
def get_most_recent_file_creation_date(directory):
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            ...
            file_creation_date = datetime.datetime.fromtimestamp(file_creation_time, tz=datetime.timezone.utc) # Convert to UTC
...
def run_process():
    ...
    try:
        response = requests.get('http://localhost:5005/get_settings')  # Adjust port if needed
    ...
    if last_processed_date:
        ...
        if last_processed_date >= most_recent_file_creation_date:
            update_progress(0, 0, 'No new updates. Last processed date is more recent than or equal to the newest file.')
            return
    ...
    if process_dir:
        ...
        total_files = sum(len(files) for files in documents.values())  # Total number of files
            ...
            for file_path in files:
                ...
                try:
                    if not ElasticsearchDatabase().check_exists(document['file_hash']):
                        ...
                        processed_count += 1
                        update_progress(processed_count, total_files, f'Processing file {processed_count} of {total_files}')
                    else:

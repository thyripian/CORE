...
#     # DatabaseManager()
#     # DatabaseManager.initialize_from_api()
#     # DatabaseManager.test_initial_connections()
#     # logger.info(f"Connection status after initialization: {DatabaseManager.connection_status}")
def on_demand_initialization(user_config):
...
def run_process():
    ...
    if process_dir:
        ...
        for ext, files in documents.items():
            for file_path in files:
                ...
                logger.info(f"FILE EXT PASSED TO PROCESSING MANAGER: {ext}")
                try:
    ...
    logger.info(f"Update process completed in {total_time:.2f} seconds.".upper())
...
#     try:
#         response = requests.get("http://localhost:5005/api/settings")  # Adjust port if needed
#         response.raise_for_status()
#         settings = response.json()
#         logger.info(f"Fetched config: {settings}")
#     except Exception as e:
...
#                         # Check if the file has already been processed
#                         if not ElasticsearchDatabase().check_exists(document["file_hash"]):
#                             processing_manager.process_data({ext: [file_path]}, processed_count + 1, total_files)
#                         else:
#                             logger.info("File already exists. Skipping.")
#                         with progress_lock:
#                             processed_count += 1

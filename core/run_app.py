...
def initialize_route():
    global flag
    if flag:
        return jsonify({"status": "success", "message": "Application is already initialized"}), 200  # Skip re-initialization
    
    try:
        generic.initialize_backend()
        flag = True
        return jsonify({"status": "success", "message": "Application initialized successfully"}), 200
    except Exception as e:
        flag = False
...
def stat():
    global flag
    if flag is None:
        return jsonify({'status': "Initialization not attempted"}), 200
    elif flag:
        return jsonify({"status": "Initialized successfully"}), 200
    else:
        return jsonify({"status": "Initialization failed"})
...
def get_config():
    try:
        if AppInitialization.initialized:
            ...
            logger.info(f"Returning elastic_conn_data: {DatabaseConfig.elastic_conn_data}")
...
def get_settings():
    try:
        if AppInitialization.initialized:
            logger.info(f"Returning settings from backend: {AppInitialization.settings}")
        ...
        return jsonify({"error": f"Unable to retrieve settings from backend: {str(e)}"}), 500
    
@app.route('/get_logfile', methods=['GET'])
def get_logfile():
    try:
        if AppInitialization.initialized:
            logger.info(f"Returning logile name from backend: {AppInitialization.logfile}")

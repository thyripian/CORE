...
class HashChecker:
    ...
    def check_hash_exists(hash_value):
        ...
        for service_key in priority_order:
            if AppInitialization.connection_status.get(service_key) or connects.get(service_key):
                ...
                if db and db.check_exists(hash_value):
                    ...
                    logger.info(f"No hash found in {service_key}. Checking Pandas DataFrame next.")
        ...
        logger.warning("Hash not found in any available databases or the Pandas DataFrame.")
        return False

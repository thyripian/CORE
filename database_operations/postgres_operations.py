...
class PostgreSQLDatabase(BaseDatabase):
    ...
    def check_exists(self,hash_value):
        ...
        with DatabaseManager.get_postgres_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("SELECT EXISTS(SELECT 1 FROM reports WHERE SHA256_hash=%s)", (hash_value,))
    ...
    def save_data(self, info):
        ...
        with DatabaseManager.get_postgres_connection() as conn:
            with conn.cursor() as cursor:
                sql = '''
                    INSERT INTO reports (SHA256_hash, highest_classification, caveats, file_path, locations, timeframes, subjects, topics, keywords, MGRS, images, full_text)

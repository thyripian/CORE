...
#         es = DatabaseManager.get_elasticsearch_client()
#         if es:
...
#                 file_path = response['hits']['hits'][0]['_source'].get('file_path', None)
#                 if file_path:
...
#             "size": 1,
#             "sort": [
#                 { "processed_time": { "order": "desc" } }
#             ],
#             "_source": ["processed_time"]
#         }
#         index_name = DatabaseConfig.elastic_index
#         res = es.search(index=index_name, body=query)  # THIS WILL NEED CHANGED FROM HARDCODED
#         if res['hits']['hits']:

...
class DatabaseManager:
    ...
    service_mapping = {
        'elasticsearch': ('database_operations.elasticsearch_operations', 'ElasticsearchDatabase'),
    ...
    def test_imports(self):
        for service_key, (module_name, class_name) in type(self).service_mapping.items():
            service_class = type(self).dynamic_import(module_name, class_name)
            if service_class:
                # instance = service_class()
                logger.session(f"{service_key} : {module_name}, {class_name} able to be set correctly.")
            else:
                logger.error(f"{service_key} : {module_name}, {class_name} unable to be set.")
    ...
    def test_initial_connections(cls):
        ...
        logger.info(f"Initial database connections tested. \n\nRESULTS:\n\t\t{cls.connection_status}")
    ...
    def try_elasticsearch_connection(cls):
        if 'elasticsearch' in cls.availability and cls.availability['elasticsearch']:
            ...
            if cls.es_client.ping():
                ...
                logger.info("Elasticsearch connection unavailable with the current user configuration")
                return False
        else:
            logger.info("Elasticsearch not avaiable as a Python import. Unable to test user configuration details.")
            return False
        ...
        if 'postgresql' in cls.availability and cls.availability['postgresql']:
            try:
                with cls.get_postgres_connection() as conn:
                    ...
                    cursor = conn.cursor()
                    cursor.execute("SELECT 1")  # Execute a simple query to test the connection
                    cursor.close()
            ...
            logger.info("PostgreSQL connection unavailable with the current user configuration.")
            return False
    ...
    def try_sqlite_connection(cls):
        if 'sqlite' in cls.availability and cls.availability['sqlite']:
            ...
            conn = connect(cls.sqlite_database)  # Ensure cls.sqlite_database is correctly initialized
            if conn:
                conn.close()
                logger.info("SQLite connection successful.")
                return True
        else:
            logger.info("SQLite connection unavailable with the current user configuration.")
            return False
    ...
    #         elastic_conn_info = DatabaseConfig.elastic_conn_data
    #         from elasticsearch import Elasticsearch
    #         cls.es_client = Elasticsearch(
    #             elastic_conn_info['url'],
    #             http_auth=(elastic_conn_info['basic_auth']['user'], elastic_conn_info['basic_auth']['password']),
    #             verify_certs=elastic_conn_info['verify_certs'],
    #             ca_certs=elastic_conn_info['ca_certs'],
    #             max_retries=1,
    ...
    def get_elasticsearch_client(cls):
        ...
        if cls.es_client is None:
            try:
                ...
                logger.info(f"ELASTIC CONN DATA COPIED FROM DB CONFIG: {elastic_conn_info}")
                ...
                logging.info(f"Elasticsearch client successfully created: {cls.es_client}")
    ...
    #             "mappings": {
    #                 "properties": {
    #                     "SHA256_hash": {"type": "keyword"},  # Unique ID, use keyword for exact matches
    #                     "highest_classification": {"type": "text"},  # Textual data, analyzed for full-text search
    #                     "caveats": {"type": "text"},  # Textual data, analyzed for full-text search
    #                     "file_path": {"type": "keyword"},  # File path, use keyword for exact matches
    #                     "locations": {"type": "text"},  # Text data, analyzed for full-text search
    #                     "timeframes": {"type": "text"},  # Text data, analyzed for full-text search
    #                     "subjects": {"type": "text"},  # Concatenated text, analyzed for full-text search
    #                     "topics": {"type": "text"},  # Concatenated text, analyzed for full-text search
    #                     "keywords": {"type": "text"},  # Comma-separated keywords, analyzed for full-text search
    #                     "MGRS": {"type": "text"},  # JSON string, analyzed for full-text search
    #                     "images": {"type": "text"},  # Base64-encoded images, stored as text
    #                     "full_text": {"type": "text"}  # Long text, analyzed for full-text search
    #                 }
    #             }
    #         }
    ...
    def get_service_module(cls, service_key):
        ...
        try:
            ...
            logger.error(f"Error importing or initializing {class_name} from {module_name}: {e}")
            return None
                ...
                if service_module:
                    ...
                    while attempt < retry_limit:
                        try:
                            ...
                            logger.error(f"Attempt {attempt + 1} to save data using {service_key} failed: {e}")
                        attempt += 1
